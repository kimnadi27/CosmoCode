...
/**
  ******************************************************************************
  * @file    main.cpp
  * @brief   PolyTest -- HDLC-слейв на базе UniversalModuleFirmware
  ******************************************************************************
  *
  * Порядок include (обязателен):
  *   1. hdlc.h               -- транспорт + DMA UART
  *   2. Utils.h              -- HDLC_Timer_t и хелперы
  *   3. PolyTestFSM.h        -- FSM poly_test (включает Motor.h, Hx711.h)
  *   4. UIHolder_*.h         -- UI-субхолдеры (команды с RS-шины)
  *   5. Holders.h            -- ПОСЛЕДНИМ
  *
  * Что НЕ нужно трогать в старом проекте:
  *   tim.c / tim.h           -- htim1/2/3 и MX_TIM*_Init остаются там
  *   adc.c / adc.h           -- не используется, оставить как есть
  *   usart.c / usart.h       -- НЕ включать: huart1 и USART1_Init теперь здесь
  *   stm32l4xx_hal_msp.c     -- ЗАМЕНИТЬ на файл из UniversalModuleFirmware
  *   stm32l4xx_it.c          -- ЗАМЕНИТЬ на файл из UniversalModuleFirmware
  *                              (нужны DMA1_Channel4/5 обработчики)
  *
  * UI-команды (UI_NORMAL 0x13, фаза READY):
  *   0xA1          -- принудительный запуск теста
  *   0xA2          -- немедленный запрос веса (без мотора)
  *   0xA3 + u32LE  -- изменить интервал (мс)
  *
  * Данные теста (RR DataBuffer, опкод 0x10), 20 байт LE:
  *   [0..3]   float  -- итоговый вес (мг)
  *   [4..7]   u32    -- шаги до порога 100 мг
  *   [8..11]  u32    -- шаги до порога 200 мг
  *   [12..15] u32    -- шаги до порога 300 мг
  *   [16..19] u32    -- шаги до порога 400 мг
  ******************************************************************************
  */
/* USER CODE END Header */
 ...
 *   hdlc.h -> Utils.h -> PolyTestFSM.h -> UIHolder*.h -> Holders.h (ПОСЛЕДНИМ)
 */
#include "../../HDLC/hdlc.h"
#include "../../Core/Inc/Utils.h"

#include "UIHolders/PolyTestFSM.h"          /* FSM + API для UIHolder'ов */
...
/* -----------------------------------------------------------------------
 * HAL-хэндлы USART1 + DMA -- нужны библиотеке UniversalModuleFirmware.
 * huart1 из старого usart.c УДАЛИТЬ или убрать usart.c из сборки,
 * чтобы не было двойного определения.
 * --------------------------------------------------------------------- */
//UART_HandleTypeDef huart1;
//DMA_HandleTypeDef  hdma_usart1_rx;
//DMA_HandleTypeDef  hdma_usart1_tx;
...
/*
 * Счётчик шагов мотора.
 * Инкрементируется в HAL_TIM_PeriodElapsedCallback (TIM3).
 * Используется FSM и классом Motor через ссылку.
 */
long long g_motor_counter = 0;

/*
 * Объекты периферии уровня приложения.
 * Инициализируются через placement-new в on_periph_enable,
 * поэтому объявляем как выровненный буфер (нет дефолтного конструктора).
 */
alignas(Hx711) static uint8_t _hx711_buf[sizeof(Hx711)];
alignas(Motor) static uint8_t _motor_buf[sizeof(Motor)];
static Hx711 *g_hx711 = nullptr;
...
//static void MX_DMA_Init(void);
//static void MX_USART1_UART_Init(void);
//static void MX_GPIO_Extra_Init(void);  /* пины мотора, HX711, 5V */
...
/**
 * on_periph_enable -- вызывается ОДИН РАЗ после первого успешного TEST.
 * Инициализируем всю периферию кроме USART1/DMA (они инициализированы до HDLC_Init).
 */
static void on_periph_enable(HDLC_Handle_t *h)
{
	return;
    ...
    /* Таймеры (htim1/2/3 объявлены в tim.h, инициализируются из tim.c) */
    MX_TIM1_Init();
    MX_TIM2_Init();
    MX_TIM3_Init();

    HAL_TIM_Base_Start(&htim1);               /* HX711 -- счётчик мкс */
    HAL_TIM_Base_Start_IT(&htim3);            /* шаги мотора, прерывание */

    /* GPIO мотора, HX711 SCK/DT, питание 5В */
    MX_GPIO_Init();
    HAL_GPIO_WritePin(GPIOB, GPIO_PIN_0, GPIO_PIN_SET); /* 5В ON */

    /*
     * Placement-new: конструируем объекты C++ в заранее выделенных буферах.
     * Это позволяет объявить указатели глобально без вызова конструкторов
     * до инициализации HAL (что вызвало бы сбой).
     */
    g_hx711 = new(_hx711_buf) Hx711(
        GPIOA, GPIOB,        /* DT_port, SCK_port */
        GPIO_PIN_12,          /* DT_pin  (PA12)   */
        GPIO_PIN_8,           /* SCK_pin (PB8)    */
        &htim1,
        8600020u,             /* tare (заводское) */
        0.24783146f           /* coefficient      */
    );

    g_motor = new(_motor_buf) Motor(
        GPIOC, GPIOC,
        GPIO_PIN_0,           /* M0 pin */
        GPIO_PIN_1,           /* M1 pin */
        g_motor_counter       /* ссылка на счётчик шагов */
    );
...
int main(void)
{
    ...
    /*
     * Только базовые GPIO (LED, RESP_GATE) и DMA+USART1 -- до HDLC_Init.
     * MX_GPIO_Init из gpio.c инициализирует LED и RESP_GATE.
     * Периферию приложения (мотор, HX711) -- в on_periph_enable.
     */
    MX_GPIO_Init();
    MX_DMA_Init();
    MX_USART1_UART_Init();
    ...
    /* ВАЖНО: HDLC_Init делает memset(h,0) -- вызывать ДО любых Register */
    HDLC_Init(&g_hdlc, &huart1, 0x41u, on_periph_enable, on_user_loop);

    UIHolder_Stats_Register(&g_hdlc);
    UIHolder_TestBuffer_Register(&g_hdlc);
    UIHolder_ForceRun_Register(&g_hdlc);
    UIHolder_GetData_Register(&g_hdlc);
    UIHolder_SetInterval_Register(&g_hdlc);
...
/** TxCplt -- уведомляем HDLC о завершении DMA-передачи */
void HAL_UART_TxCpltCallback(UART_HandleTypeDef *huart)
{
    if (huart == g_hdlc.huart)
        HDLC_Transmitter_TxDone(&g_hdlc);
...
/**
 * TIM3 period elapsed -- инкрементируем счётчик шагов мотора.
 * Реализация HAL_TIM_PeriodElapsedCallback уже есть в HAL слабым символом,
 * поэтому перекрываем её здесь.
 */
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
    if (htim->Instance == TIM3)
...
//
///**
// * GPIO мотора, HX711, питание 5В.
// * Вызывается из on_periph_enable, ПОСЛЕ HAL_Init и SystemClock_Config.
// * LED и RESP_GATE уже настроены в MX_GPIO_Init (из gpio.c).
// */
//static void MX_GPIO_Extra_Init(void)
//{
//    GPIO_InitTypeDef GPIO_InitStruct = {0};
...
/* -----------------------------------------------------------------------
 * Тактирование (сгенерировано CubeMX, не трогать)
 * --------------------------------------------------------------------- */
void SystemClock_Config(void)
{
    RCC_OscInitTypeDef RCC_OscInitStruct = {0};
